...
- A complete interval of uniformly below-random lags packs: `|U|(L+c+1) ≤ (c+1)q` (Lean: `pack_window`, including the window expansion). Average-only / `K=2` discrepancy and spread-out neighbourhoods do not.
...
- Medium aligned fibres ($|B_x|\ge q/3$) have a $1/2$-close star (`inverse-energy.tex`). This is structure, not a size bound, except when the neighbourhood is a complete interval with a uniform below-random lag bound (Lean: `pack_window`). Half-intersection does not beat the random overlap when $|U|\le q/2`.
