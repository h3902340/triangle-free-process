...
- Exact cylinders over a $T$-interval pack by the interval lemma (Lean: `exact_cylinder_pack`).
- Three-quarters of maximal energy produces a high-multiplicity core at any fibre size (Lean: `three_quarters_energy_core`).
...
1. **Intermediate-energy fibres** of size in $(q^{1/3}(\log q)^{-O(1)}, q/3)$, including thin three-quarters-energy stars that are not exact cylinders. Medium fibres cluster; exact cylinders pack by the interval lemma (Lean: `exact_cylinder_pack`). Three-quarters of maximal energy produces a $1/2$-close star at any fibre size (Lean: `three_quarters_energy_core`); a thin such star does not pack elementarily. Half-intersection is vacuous for thin cores. Fibres $\le q^{1/3}/(\log q)^{O(1)}$ cannot prevent an Alon-beating bound after the lift.
