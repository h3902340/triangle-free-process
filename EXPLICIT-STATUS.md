...
2. **Medium-star incidences / light-star second moment.** An oversized $M_I$ reduces to high seed energy of $\pi_R(I)$ and does not open a third gap (`open-edges.tex`). Light stars are under control at the weaker target $|I|\ge C\sqrt{n}\,(\log n)^{5}$ with $t_3=(\log n)^{2}$. The SOTA scale still needs a second moment. The first-moment half is Lean (`seedDegree_sum`, `seed_heavy_star_count`, `light_seed_pairs`): $\sum D=|S_R||P|$ and $2\sum_{D\le t}\binom{D}{2}\le t|S_R||P|$, which is $o(|P|^2)$ only for $t|S_R|\ll|P|$. The Sidon count $\sum D(u)^2\le|S_R||P|+6|P|(|P|-1)$ is Lean (`seedDegree_sq_sum`) and is not $o(|I|^2)$ at $|I|\sim\Delta$.
