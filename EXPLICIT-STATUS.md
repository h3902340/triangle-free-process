...
- A complete *affine* interval of uniformly below-random lags packs: `|U|(L+c+1) ≤ (c+1)q` (Lean: `pack_window`, including the window expansion). A `3/4`-close in-star on `{t+1,…,t+L}` is such an interval (Lean: `pack_quarter_in_star`). High-vertex out-neighbourhoods produce quadratic lags and do not pack this way. Average-only / `K=2` discrepancy and spread-out neighbourhoods do not.
...
- Medium aligned fibres ($|B_x|\ge q/3$) have a $1/2$-close star (`inverse-energy.tex`). This is structure, not a size bound, except for a `3/4`-close in-star whose high neighbours contain `{t+1,…,t+L}` (Lean: `pack_quarter_in_star`). A high-vertex $N^+$ is interval-dense on $T$-dense $x$, but those lags are quadratic. Half-intersection does not beat the random overlap when $|U|\le q/2`.
