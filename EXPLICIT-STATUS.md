...
1. **Intermediate-energy fibres** of size in $(q^{1/3}(\log q)^{-O(1)}, q/3)$. The window $[q/3,q/2]$ is empty of intermediate energy: alignment there is two-thirds energy (Lean: `aligned_implies_two_thirds`, `aligned_medium_has_core`), and size $q/2$ cannot be strictly aligned (Lean: `no_aligned_of_half_fibres`). Medium aligned fibres have a high-multiplicity core. On a $T$-dense $x$ the oriented star neighbourhood is interval-dense (Lean: `intervalT_out_mem`, `card_intervalT_out`); the remaining gap for those stars is Weyl. The leftover is thin two-thirds-energy stars that are not exact, thin AP-poor two-clusters ($|T_i|=o(d)$), and those unpacked medium stars. Exact cylinders (any AP support) pack by the interval lemma (Lean: `exact_cylinder_pack_ap`). Fibres $\le q^{1/3}/(\log q)^{O(1)}$ cannot prevent an Alon-beating bound after the lift.
