...
\begin{itemize}
  \item intermediate-energy fibres of size in $(q^{1/3}(\log q)^{-O(1)},3q/8)$ (\texttt{inverse-energy.tex}), including thin three-quarters-energy stars that are not exact, AP-poor two-clusters, and medium $1/2$-close stars of mean size in $[q/3,3q/8)$ (half-intersection does not pack non-heavy cores); the window $[3q/8,q/2]$ is empty; exact (AP-support) cylinders pack;
