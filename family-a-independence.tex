...
\begin{itemize}
  \item intermediate-energy fibres of size in $(q^{1/3}(\log q)^{-O(1)},q/3)$ (\texttt{inverse-energy.tex}), including thin two-thirds-energy stars that are not exact, AP-poor two-clusters, and medium $1/2$-close stars (half-intersection does not pack non-heavy cores); the window $[q/3,q/2]$ has no intermediate energy (alignment is two-thirds energy and a high-multiplicity core); exact (AP-support) cylinders pack;
