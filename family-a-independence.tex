...
Until that lemma is proved, Family~A does not yet give a new explicit Ramsey bound.
\end{abstract}
...
\begin{proof}
If $r\sim r'$ in $G_R$ and $(r,i),(r',j)\in I$ for any shears $i,j$, then $(r,i)(r',j)$ is a red $G_2$-edge.
Hence a $G_2$-independent set meets the fibre of at most one endpoint of every $G_R$-edge, i.e.\ $\pi_R(I)$ is $G_R$-independent.
The blue statement is identical after applying the shears (each blue vertex has $\ell$ preimages, but adjacency depends only on the blue coordinates).
...
The missing pieces are Conjecture~\ref{conj:seed} (an additive-combinatorial bound for one Cayley graph on $\F_q^{2}$) and Lemma~\ref{lem:remaining} (open edges in large sets).
Both are concrete and, in our view, true; they are not written out at the level of a refereed proof.
