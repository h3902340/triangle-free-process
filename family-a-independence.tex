...
\begin{itemize}
  \item intermediate-energy fibres of size in $(q^{1/3}(\log q)^{-O(1)},q/3)$ (\texttt{inverse-energy.tex}), including thin three-quarters-energy stars that are not exact, AP-poor two-clusters, and medium $1/2$-close stars (half-intersection does not pack non-heavy cores); exact (AP-support) cylinders pack;
