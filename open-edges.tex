...
The same first-moment comparison on the seed graph is Lean
(\texttt{seedDegree\_sum}, \texttt{seed\_heavy\_star\_count}):
$\sum_u D(u)=|S_R||P|$ and $t\cdot\#\{u:t\le D(u)\}\le|S_R||P|$.
\end{proof}
...
The same comparison is Lean (\texttt{two\_mul\_sum\_choose\_two\_le},
\texttt{light\_seed\_pairs}):
$2\sum_{D\le t}\bin{D}{2}\le t|S_R||P|$.
This is $o(|P|^{2})$ only when $t|S_R|\ll|P|$; at the SOTA scale
$|P|\sim|S_R|$ that needs $t=o(1)$.
\end{proof}
...
The unsigned seed codegree is Lean-verified (\texttt{parabola\_codegree}): a nonzero $\delta\in\F_q^{2}$ has at most one representation as a difference of unsigned parabola points.
The signed connection set has codegree at most $6$
(Lean: \texttt{signedParabola\_codegree}), and the Cauchy--Schwarz count
\[
  \sum_u D(u)^{2}\;\le\;|S_R|\,|P|+6|P|(|P|-1)
\]
is Lean (\texttt{seedDegree\_sq\_sum}).
That is $O(d|P|+|P|^{2})$, which is not $o(|I|^{2})$ at the SOTA scale
(see the parameter discussion in the remark after Lemma~\ref{lem:light}).
...
$\sum_u D(u)^{2}=O(d|P|+|P|^{2})$
(Lean: \texttt{seedDegree\_sq\_sum}) is still not $o(|I|^{2})$ at
...
Lemma~\ref{lem:med} (Lean: \texttt{seedDegree\_sum},
\texttt{light\_seed\_pairs}).
