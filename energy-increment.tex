...
A complete interval of uniformly below-random lags does pack
...
\begin{enumerate}
  ...
  \item a leftover of intermediate-energy fibres of size in
        ...
        bound, except in that complete-interval uniform case.
\end{enumerate}
Items (1)--(4) are theorems (no Weyl).
Item (5) is a structure theorem, not a size bound, except for a complete
interval of uniformly below-random lags: half-intersection does not
beat the random overlap on non-heavy cores, so the horizontal-packing Weyl
estimate does not apply to $K=2$ stars.
