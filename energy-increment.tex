...
or is a medium $\tfrac12$-close star that is average-only or spread-out,
or is a thin AP-poor two-cluster),
...
A complete affine AP of uniformly below-random lags does pack
(\texttt{inverse-energy.tex}, Lemma~2ndmoment; Lean
\texttt{pack\_window}); a $\tfrac34$-close in-star along any common
difference is such an AP (Lean \texttt{pack\_quarter\_in\_star\_ap}).
...
thin AP-poor supports ($|T_i|=o(d)$) remain leftover.
On a $T$-dense medium star, AP-poor support is not a separate
obstruction: the oriented neighbourhood is interval-dense
(\texttt{inverse-energy.tex}, Lemma~stardense).
...
\begin{enumerate}
  ...
  \item a leftover of intermediate-energy fibres of size in
        ...
        (\texttt{inverse-energy.tex}, Lemma~2ndmoment; Lean
        \texttt{pack\_window}); a $\tfrac34$-close in-star along any
        common difference is such an AP (Lean
        \texttt{pack\_quarter\_in\_star\_ap}).
        Medium aligned fibres have two-thirds energy
        and a high-multiplicity core; that is structure, not a size
        bound, except for that in-AP case.
\end{enumerate}
Items (1)--(4) are theorems (no Weyl).
Item (5) is a structure theorem, not a size bound, except for a
$\tfrac34$-close in-AP of uniformly below-random lags:
half-intersection does not beat the random overlap on non-heavy cores,
a high-vertex $N^+$ produces quadratic lags, and the
horizontal-packing Weyl estimate does not apply to $K=2$ stars.
