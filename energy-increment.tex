...
If they have high energy, a positive-density piece is aligned to a common translate of the parabola and reduces to a horizontal packing of size $O(q\log q)$.
...
That argument, exact-cylinder packing by the interval lemma, three-quarters
energy at any fibre size, and the leftover intermediate-energy window, are in
\texttt{inverse-energy.tex}.
Together with the structured-case list in \texttt{structured-cases.tex}, this disposes of every independent set we can describe except intermediate-energy fibres of size in $(q^{1/3}(\log q)^{-O(1)},q/3)$ and thin three-quarters-energy stars that are not exact.
\end{abstract}
...
\]
In particular $U$ is disjoint from $\Omega(|T_x|)$ of its own translates, hence $|U|=O(\log q)$ by the same Weyl estimate as in the horizontal-packing case of \texttt{structured-cases.tex}.
The cylinder therefore contributes $O(q\log q)$ points.
\end{lemma}
...
If $2W_x^{2}/q<E_x<\tfrac34|T_x|W_x$ and every fibre at $x$ has size in $\bigl(q^{1/3}(\log q)^{-O(1)},q/3\bigr)$
(or the fibre has three-quarters energy but is only $\tfrac12$-close to a thin core),
...
Medium fibres of size at least $q/3$ \emph{do} cluster: see Theorem~8 of \texttt{inverse-energy.tex}.
Exact cylinders pack by the interval lemma, without Weyl
(Lean: \texttt{exact\_cylinder\_pack}).
Three-quarters of maximal energy produces a $\tfrac12$-close star at any fibre
size (Lean: \texttt{three\_quarters\_energy\_core}); a thin such star does not
pack elementarily, and is part of the leftover.
Half-intersection is vacuous for thin cores; twice-random energy does not force constant-fraction overlap below density $1/4$.
...
\begin{enumerate}
  ...
  \item medium approximate cylinders, each of size $O(q\log q)$ (\texttt{inverse-energy.tex}),
        including three-quarters-energy stars with core size $\ge q/3$;
...
\end{enumerate}
Items (1)--(5) are theorems (item (5) uses the same Weyl input as horizontal packings).
Item (6) is Conjecture~\ref{lem:inverse}.
\end{theorem}
...
Everything else in Theorem~\ref{thm:dichotomy} is elementary or uses the same Weyl input as the horizontal-packing case.
